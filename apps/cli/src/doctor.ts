...
export async function inspectContextWeft(startPath: string): Promise<DoctorReport> {
  ...
  /* v8 ignore next -- @preserve process.versions.node always has a major segment in supported Node. */
  const major = Number.parseInt(process.versions.node.split(".")[0] ?? "0", 10);
  /* v8 ignore next -- @preserve CI and supported local development run on Node.js 22 or newer. */
  ...
  try {
    ...
    checks.push({
      ...
      /* v8 ignore next -- @preserve repository construction migrates local stores to the latest schema. */
...
function errorMessage(error: unknown): string {
  /* v8 ignore next -- @preserve callers pass Node/SQLite Error objects in production paths. */
